...
int recvFile(FILE *fd)
{
	...
	printf("Receiving...\n");
	char buffer[123431];
	int index=0, max_size = strlen(buffer);
	...
	while(1) 
	{
        ...
        if (recvfrom(sockfd, &rcv_pkt, sizeof(rcv_pkt), 0, (struct sockaddr *)&info, (socklen_t *)&len)) {
            if(isLoss(0.5))
            {
            ...
            int now_length = (rcv_pkt.header.is_last) ? 551 : 1024;

            //==============================================
		    // Actually receive packet and write into buffer
		    //==============================================
            memcpy(buffer + receive_packet, rcv_pkt.data, now_length);
            receive_packet += now_length;

            //====================
            // Reply ack to server
            //====================
            snd_pkt.header.ack_num = rcv_pkt.header.seq_num;
            snd_pkt.header.seq_num = index++;
